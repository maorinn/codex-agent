...
优先使用 non-interactive 模式（简单、可恢复、易自动化）。

#### 方案 A：exec（推荐）

| 场景 | 命令 |
|------|------|
| 首次执行 | `nohup codex exec --full-auto -C <工作目录> "<提示词>" > /tmp/codex_exec_output.txt 2>&1 &` |
| 继续最近一次 | `codex exec resume --last "<后续指令>"` |
| 精确恢复 | `codex exec resume <SESSION_ID> "<后续指令>"` |
| 跨目录恢复 | `codex exec resume --all <SESSION_ID> "<后续指令>"` |
...
# 如需切换模型
tmux send-keys -t codex-<任务名> '/model gpt-5.3-codex xhigh'
sleep 1
